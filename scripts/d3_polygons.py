...
# Get polygons
# Generate watershed upstream of the confirmed barrier from D1 with the most stream potentially accessible to salmon
watershed = geopandas.read_file(
    "https://features.hillcrestgeo.ca/fwa/functions/postgisftw.fwa_watershedatmeasure/items.json?blue_line_key=360788426&downstream_route_measure=1203"
).to_crs("EPSG:3005")
...
watershed.plot(
    ax=ax, color="#f85a65", alpha=0.6, zorder=100
)  # ensure streams are plotted on top.
